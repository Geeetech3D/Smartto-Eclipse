...
void Recovery_sdprint(void)
{
	...
	u8 res,i=0;
      ...
      if(recovery_parameter.Current_sd_byte != 0)
      {
		...
		Processing_command();
  		sprintf(Command_Buffer,"M190 S%2f\r\n",Setting.targe_temperature[BED]);  //设置热床温度，但是不等待
  		Processing_command();
  		...
  		else
  		{
  			...
  			if(system_infor.Auto_Levele_Flag ==1 )
			{
		    		Current_Position[Z_AXIS] =pos[Z_AXIS];
		  	       position[Z_AXIS]= pos[Z_AXIS]*Setting.steps_per_mm[Z_AXIS] ;
			}
			else
			{
			  	test_times=3;
		...
		Enable_all_Axis();//以防加热时间过长电机解锁，使能所有电机
...
void Recovery_create(void)
{
  ...
  else
  {
              ...
              STMFLASH_Write2(RECOVERY_SAVE_ADDR,(u16*)(&Recovery_Save_Data),sizeof(Recovery_Save_Data)/2); //优先保存断电信息sizeof(bp)/2
