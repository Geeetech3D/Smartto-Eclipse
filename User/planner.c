...
{
    char test_Str[500];
    static char Num_Unexpected_events=0;//连续出现的话停止
    u16 text_length;
  u8 i;
  ...
  {
 	block->bed_ForCalulate_point_x=x;//作为断电续打数据
	block->bed_ForCalulate_point_y=y;//要运行到的位置
  ...
  if(system_infor.Auto_Levele_Flag==1)
  {
	float Cal_delta_mm[4];
                              //block_buffer_heady有取值范围，注意，不能用block_buffer_head-1，否则会有负数出来
	...
	if((system_infor.sd_print_status==0x02||system_infor.serial_printf_flag == 1)&&system_infor.auto_leveling_calculate_enable==ENABLE)//打印时回抽长度大于15mm，会有退料的风险
	{
       	...
       	if((Cal_delta_mm[E_AXIS]<-20.0)||Cal_delta_mm[E_AXIS]>(Setting.max_position[X_AXIS]+Setting.max_position[Y_AXIS])||(Cal_delta_mm[X_AXIS]==0.0&&Cal_delta_mm[Y_AXIS]==0.0&&Cal_delta_mm[E_AXIS]>20.0)||(block->sd_byte>5120&&Cal_delta_mm[Z_AXIS]<-0.5&&block->bed_ForCalulate_point_z>1.5)) 
		{
			...
			if(system_infor.sd_print_status==0x02){//突发事件Unexpected_events
				//fptr_buf=pre_sd_byte;//作为错误复读位置
				if(Num_Unexpected_events<8)//连续8次出现的话停止打印
                                {
                                	  if(test_times<2)
                                	  {
   ...
   if(system_infor.Auto_Levele_Flag!=1)
   {
	if(system_infor.sd_print_status==0x02||system_infor.serial_printf_flag == 1)//打印时回抽长度大于15mm，会有退料的风险
	{
		 //退料长度大于20mm
		//进料长度大于两边长度
		//单独E轴进料长度大于20mm
  ...
  block->acceleration_rate = (float)block->acceleration_st / F_TIM1;
	...
	float vmax_junction = Setting.max_jerk; //设定默认的连接速度为最大突变速度
	float vmax_junction_factor = 1.0;            //速度链接因子
  ...
  block->entry_speed = MIN(vmax_junction, v_allowable);
	...
	memcpy(block->position_block, position, sizeof(block->position_block)); //保存计算出来的当前位置的步数
...
static void Axis_go_origin(u8 axis,u8 dir, u16 speed, u8 home, float origin_coordinate )    //axis:轴；dir:方向；speed:速度; home:归位后的实际位置，origin_coordinate:归位后位置值
{    	  
    u16 i,distance;
	u8 aa=0;
    ...
    if((Autohome.count%Autohome.period[axis] == 0)&&(Autohome.flag[axis])) 
    {
      ...
      else if(axis == Z_AXIS) 
      {
       ...
       if(Autohome.step[axis] == 0)
       {
          ...
          //if(get_endstop_state(axis,home) == touch_end_signal[axis])  //如果已接触限位开关则先移开
         
           if(endstop_status[axis] == touch_end_signal[axis])  
          { 
         
              Autohome.step[axis] = 1;
              Autohome.move_distance[axis] = distance*4;   //设置轴离开Endstop位置为4mm 
          }  
          else Autohome.step[axis] = 2;
       }
       if(Autohome.step[axis] == 1)  //轴离开Endstop位置4mm
       {
          ...
          if(Autohome.move_distance[axis] ==0) 
          {
            ...
            Autohome.period[axis] = Autohome.period[axis]*4;//速度降低，使定位精准
          }
       }
        if(Autohome.step[axis] == 2)
        {
           ...
           if(endstop_status[axis] != normal_end_signal[axis]) 
           {
              ...
              Autohome.move_distance[axis] = distance*4;//设置轴离开Endstop位置为2mm 
           }
        }
      
       if(Autohome.step[axis] == 3) //轴离开Endstop位置2mm
       {
       ...
       if(Autohome.step[axis] == 4) 
       {
           ...
           if(endstop_status[axis] != normal_end_signal[axis]) 
           {
              ...
              Autohome.move_distance[axis] = distance/2;//设置轴超过Endstop位置0.5mm
           } 
       }    
       if(Autohome.step[axis] == 5) //轴越过Endstop位置0.5mm
       {
          ...
          if(Autohome.move_distance[axis] ==0) 
          {
            ...
            Autohome.flag[axis] = 0;  //Auto Home结束         
	     axis_known_position[axis]=true;//归位完成状态保存
          }
       }
...
long st_get_position(u8 axis)
{
  long count_pos;
  //CRITICAL_SECTION_START;
  count_pos = count_position[axis];//后面添加，可能会修改
  //CRITICAL_SECTION_END;
  return count_pos;
}
...
{
  //apply_rotation_xyz(Setting.plan_bed_level_matrix, &x, &y, &z);//注意，这个函数和marlin不一样

  position[X_AXIS] = lround(x*Setting.steps_per_mm[X_AXIS]);//看看是不是和原来的冲突
  ...
  st_set_position(position[X_AXIS], position[Y_AXIS], position[Z_AXIS], position[E_AXIS]);//记得添加count_position的代码
