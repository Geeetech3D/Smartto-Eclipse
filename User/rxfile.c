...
extern u8 SD_Data_Buffer[SD_DATA_BUF_SIZE];
...
void M2100_CMD(void)
{
        u16 len=0;
	Get_RxData_Size();
	len=crc16(Command_Buffer,File_DownST.RxData_Len-2);
	Analysis_RxData();
}
...
extern char uart_buff[UART_BUFF_SIZE],uart_buff3[UART3_BUFF_SIZE];
...
void WDGOpen_Rxfile(void)
{
	...
	TIM_TimeBaseStructure.TIM_Period = 39999;      // 4s中断40000*7200/72000000
...
extern u8 TransferError_flag;//传输出错时退出处理
...
u8 AddData_To_File(char *data,u16 len,char *filename,long long offset)
{
    ...
    u8 res =0,res1=0;
    //char str[1024]  ;
//    char str2[300]="I am liu hong fei ,zai zhenj sss ddd fff gggg qqqq  wwww\0\r\n";
//   if( (f_mount(&fats,SD_Driver_Number,1)) == SD_OK)
	...
	if(res1||fnew3.fptr != offset)//检查文件大小是否已被正确扩展
	  {     f_close(&fnew3); 	
...
u8  Sent_Boot_Binfile(void)
{
	...
	u16 read_size=0,crc_volues,n=0,file_crc_sum=0;
       u8 f_status4,f_status1,f_status2,f_status3,buf[512];//buf2[512];
	char file_pathss[60];
	...
	//sprintf(file_path,"SD1:/%s",file_path);
	clear_Uart();
	while(1)
	{
		switch(Send_data_state)
		{
			case S_HEAD:
				 ...
				 Delay_Times=0;
                                 Send_Times=0;
                                 Send_flag=1;
                          // f_open(&fs1,"SD1:/e180.bin",FA_CREATE_ALWAYS|FA_WRITE );
				break;
			case W_HEAD:
				if(Delay_Times>40)
				{
...
void  Sent_Boot_Binfile1(void)
{
	u32 Print_File_Br;
	u8 res =0,i=0,f_status,buf[512];
	char *Binfile_buf,*RBinfile_buf;
	char *sp;
	char str[1024] ;
	//Binfile_buf=(char *)malloc(1024);
	//RBinfile_buf=(char *)malloc(1024);
	u32 off_add=0;
	u16 read_size=0,crc_volues,n=0;
	char binname[50],binpath[60];
	u8 static Re_Send_Times=0,send_st=0,Fnum=0;
	res = f_open(&fnew3,"SD1:/config.txt",FA_OPEN_EXISTING|FA_READ );// FA_OPEN_ALWAYS | |FA_READ
	 if(res == FR_OK)
	 {
		 f_status = f_lseek(&fnew3,0);
     		 f_status = f_read(&fnew3,SD_Data_Buffer,fnew3.fsize, &Print_File_Br);
		 f_close(&fnew3);	
		 if(strstr(SD_Data_Buffer,"boot:1")!=NULL)
		 {
                        memset(binname,0,50);
		 	sp=strstr(SD_Data_Buffer,"file_path:");
		 	sp=sp+10;
		 	for(i=0;i<50;i++)
		 	{
				if(*sp==';')
				...
				sp++;
                        ...
                        printf("%s\r\n",binname);
			if(strstr(binname,"bin")!=NULL)
			{       memset(binpath,0,60);
				sprintf(binpath,"SD1:/%s",binname);
			}
			else
			   return;
		 	if( f_open(&fnew3,binpath,FA_OPEN_EXISTING|FA_READ )==FR_OK)// FA_OPEN_ALWAYS | |FA_READ
		 	   printf("%s\r\n",binpath);
			else
			   return;
		 }
	...
	printf("%s",str);
	 ...
	 while(1)
	 {
		...
		{
			case 0:   //读取文件数据
				if(fnew3.fsize>(off_add+512))
					read_size=512;
				else
				{
					read_size=fnew3.fsize-off_add;
					finish_boot_send();
				}
				
			 	 f_status = f_lseek(&fnew3,off_add);
				 f_status = f_read(&fnew3,SD_Data_Buffer,read_size, &Print_File_Br);

				  f_status = f_lseek(&fnew3,off_add);
		                  f_status = f_read(&fnew3, buf, read_size, &Print_File_Br);
				   //printf("HH:%s:kkk\r\n",SD_Data_Buffer);
				  if(strncmp(SD_Data_Buffer,buf,read_size)==0)
		          	  {
				 	crc_volues=crc16(SD_Data_Buffer,read_size);
					//sprintf(str,"<M2121 file:%s;off:%d;size:%d;data:%s;CRC:%d;>\r\n",binname,off_add,read_size,SD_Data_Buffer,crc_volues);
		                     sprintf(str,"<M2121 file:%s;off:%d;size:%d;CRC:%d;data:",binname,off_add,read_size,crc_volues);
					send_st=1;
			...
			case 1:   //发送数据
				...
				{
					   USART_SendData(USART3, (unsigned char) SD_Data_Buffer[n]);
				...
				break;
			case 2:  //等待应答
                          if(strstr(uart_buff,"M2121 ok")!=NULL)
                          	{
                          		Re_Send_Times=0;
					off_add=off_add+read_size;
                                   send_st=0;
                          	}
			    if(strstr(uart_buff,"M2121 resend")!=NULL)
			    {
				send_st=0;
			    }
                          if(Fnum>10)
						...
						break;
                          ...
                          Fnum++;
				if(fnew3.fsize<=off_add)
				    return;
                             delay_ms(50);
				break;
			case 3:  //chongfa
				 printf("<M2121 start:%s;>\r\n",binname);
				 send_st=2;
				 Fnum=0;
			  break;
		}
		...
		res = f_read(&fs,SD_Data_Buffer,data_size, &Print_File_Br);
		 ...
		 if(strstr(SD_Data_Buffer,"boot:1")!=NULL)
		 {
			...
			if(strstr(file_name,"bin")!=NULL)
			{
				 ...
				 //if( f_open(ffs,file_path,FA_OPEN_EXISTING|FA_READ )==FR_OK)
				 //{
					ret=1;
				 //}
			}
...
void Send_Data_BIN(u8 *data,u16 size)
{
