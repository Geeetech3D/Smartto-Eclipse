...
{
  const double value = 2.220446049250313E-016;//浮点数有限制精度，它取决于系统，这个值代表接近0
  return value;
}
...
/**
  ...
  Parameters:
    ...
    Input, int M, N, the number of rows and columns.//行与列

    Input, double A1[M*N], the matrix to be copied.//被复制的矩阵

    Output, double R8MAT_COPY_NEW[M*N], the copy of A1.//被复制出来的矩阵
*/
...
/**
  Purpose:
    ...
    QR_SOLVE solves a linear system in the least squares sense.QR分解解决线性最小二乘法问题
  ...
  Parameters:
    ...
    Input, int M, the number of rows of A.//A矩阵的行数M

    Input, int N, the number of columns of A.//A矩阵的列数N

    Input, double A[M*N], the matrix.//A矩阵

    Input, double B[M], the right hand side.//在右手边的

    Output, double QR_SOLVE[N], the least squares solution.//最小二乘方的解QR_SOLVE[N]
*/
void qr_solve(double x[], int m, int n, double a[], double b[])
{
  double *a_qr, *qraux, *r, tol;
  int ind, itask, *jpvt, kr, lda;
  
  a_qr = ( double * ) malloc ( n * m * sizeof ( double ) );//开辟一m*n个double型空间
  jpvt = ( int * ) malloc ( n * sizeof ( int ) );
  qraux = ( double * ) malloc ( n * sizeof ( double ) );
  r = ( double * ) malloc ( m * sizeof ( double ) );


  r8mat_copy(a_qr, m, n, a);//复制矩阵a到新的矩阵a_qr
  lda = m;
  tol = r8_epsilon() / r8mat_amax(m, n, a_qr);//返回矩阵a_qr的最大绝对值
  itask = 1;
