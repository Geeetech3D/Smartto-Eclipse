...
static char Command_Buffer_befor[200];	//前Gcode命令缓存
char com_cpy[200];	//Gcode命令缓存
extern char Command_Buffer[CMDBUF_SIZE];	//Gcode命令

u16 fan_current_speed;	//风扇当前速度
#define ROUND_TO_UINT16(x)   ((u16)(x)+0.5)>(x)? ((u16)(x)):((u16)(x)+1) //将浮点数x四舍五入为




float new_Z_Max_Position;//Z轴打印范围
...
u8 Leveling_SetMotor_flag = DISABLE;//调平电机状态

extern float Current_Temperature[5];	//当前温度
...
float Leveling_Page_Zadjust(u8 tt,u8 spaces)
{
      ...
      else    if(tt==3)
    {
           ...
           if(new_Z_Max_Position-space_values[spaces]>=0)
               new_Z_Max_Position -= space_values[spaces];
           else
