...
static void Temperature_PWM_Mode_Config(void)
{
  ...
  TIM_TimeBaseStructure.TIM_Period = 4095;       //当定时器从0计数到999，即为1000次，为一个定时周期
  TIM_TimeBaseStructure.TIM_Prescaler = 35;	    //设置预分频：不预分频，即为2MHz
  ...
  TIM_OCInitStructure.TIM_Pulse = Default_Val;	   //设置跳变值，当计数器计数到这个值时，电平发生跳变
  ...
  TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable;
  TIM_OCInitStructure.TIM_Pulse = Default_Val;	  //设置通道的电平跳变值，输出另外一个占空比的PWM
  TIM_OC2Init(TIM2, &TIM_OCInitStructure);	  //使能通道
  TIM_OC2PreloadConfig(TIM2, TIM_OCPreload_Enable);

  TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable;
  TIM_OCInitStructure.TIM_Pulse = Default_Val;	  //设置通道的电平跳变值，输出另外一个占空比的PWM
  TIM_OC3Init(TIM2, &TIM_OCInitStructure);	  //使能通道
  ...
  /* PWM1 Mode configuration: Channel2 */
  TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable;
  TIM_OCInitStructure.TIM_Pulse = Default_Val;	  //设置通道2的电平跳变值，输出另外一个占空比的PWM
  TIM_OC2Init(TIM3, &TIM_OCInitStructure);	  //使能通道
  ...
  TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable;
  TIM_OCInitStructure.TIM_Pulse = Default_Val;	  //设置通道的电平跳变值，输出另外一个占空比的PWM
  TIM_OC2Init(TIM4, &TIM_OCInitStructure);	  //使能通道 
  TIM_OC2PreloadConfig(TIM4, TIM_OCPreload_Enable); 

  TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable;
  TIM_OCInitStructure.TIM_Pulse = Default_Val;	  //设置通道的电平跳变值，输出另外一个占空比的PWM
  TIM_OC3Init(TIM4, &TIM_OCInitStructure);	  //使能通道 
...
static u16 PID_adjust(u8 e)
{
  ...
  if(error > (Setting.pid_adjust_range-5)&&e==3)//热床加温
  {
    pwm_output = FULL_POWER;
    Pid_Reset[e] = SET; 
  }
  else if((error > Setting.pid_adjust_range)&&e!=3)//Setting.pid_adjust_range&&&&e!=3)挤出头加温
  {
      pwm_output = FULL_POWER;
      Pid_Reset[e] = SET; 
  }
...
void Gloabl_temperature_control(void)
{
  ...
  for(unsigned char e=0;e<HOTHEAD_NUMBER;e++)
  {
    ...
    {    
      ...
      {
          if(Current_Temperature[e] > Setting.max_temperature[e]+10)  //如果温度高于最大值，停止加热
          {
              Temperature_Control_PWM[e] = SHUT_OFF_POWER;
          }
          
          else if(Current_Temperature[e] < Setting.min_temperature[e])//如果温度低于最低值。满功率加热
          {
              Temperature_Control_PWM[e] = FULL_POWER;
          }
          
          else//温度在设定温区内进行PID调节
          {
...
void Temperature_Handler(void)
{
  ...
  if(time_count >= Setting.temperature_sampling_period * 100)// 0.3s检查控制一次温度
  {
    time_count=0;
