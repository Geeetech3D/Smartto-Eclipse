...
u32 normal_end_signal[6]={0,0,0},touch_end_signal[6]={1,1,1},endstop_status[3]={0,0,0};//, jitter_time[3] = {0,0,0};
extern setting_t Setting;
extern char Printf_Buf[128];
u32 normal_filament_signal=0, filament_status=0, filament_jitter_time = 0;
...
void Read_endstop(void)
{
 ...
 // my_printf("signal[X_AXIS]=%d,%d\r\n",signal[X_AXIS],Setting.home_position[X_AXIS]);
  ...
  if(Setting.home_position[Z_AXIS] == MINENDSTOP) signal[Z_AXIS] = GPIO_ReadInputDataBit(Endstop,Min_Z);
	...
	//endstop_status[0] = signal[0];
	//endstop_status[1] = signal[1];
	//endstop_status[2] = signal[2];
 /*   for(i=0;i<3;i++)
  {
    ...
    if(endstop_status[i] != signal[i])
    {
	 ...
	 endstop_status[X_AXIS] = signal[X_AXIS];//GPIO_ReadInputDataBit(Endstop,Min_X);
	 endstop_status[Y_AXIS] = signal[Y_AXIS];//GPIO_ReadInputDataBit(Endstop,Min_Y);
	 endstop_status[Z_AXIS] = signal[Z_AXIS];//GPIO_ReadInputDataBit(Endstop,Min_Z);
...
void Endstop_Level_Init(u8 axis, u8 endstop)
{
    ...
    my_printf("axis=%d, t=%d,n=%d\r\n",axis, touch_end_signal[axis],normal_end_signal[axis]);
