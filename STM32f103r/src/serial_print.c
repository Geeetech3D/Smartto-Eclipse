              ...
              if(strchr(cmdbuffer[bufindw], 'N') != NULL)
              {
                ...
                if(gcode_N != gcode_LastN + 1)   //Check the Line Number
                {
                  ...
                  {                            
                    ...
                    my_printf("Error: ");
                    my_printf("Line Number is not Last Line Number+1, Last Line: ");
                    my_printf("%s\r\n",gcode_line);
                    my_printf("Resend: ");
                ...
                if(strchr(cmdbuffer[bufindw], '*') != NULL)  //check the checksum
                {
                  ...
                  if(checksum != (u16)strtol(&Command_Buffer[strchr_pointer - Command_Buffer + 1], NULL, 10)) 
                  {
                    ...
                    my_printf("Error: ");
                    my_printf("checksum mismatch, Last Line: ");
                    my_printf("%s\r\n",gcode_line);
                    my_printf("Resend: ");
                    sprintf(gcode_line,"%ld",gcode_N);
                    my_printf("%s\r\n",gcode_line);
...
void fetch_next_command(void)
{
    ...
    {         
        ...
        CntCmdProc -=1; 
        Processing_command();
        if(system_infor.serial_ack_flag) my_printf("ok\r\n");
    }
}
