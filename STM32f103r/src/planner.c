...
extern u32 normal_end_signal[6], touch_end_signal[6], endstop_status[6];
...
void Plan_buffer_line(float x, float y, float z, const float e, float feed_rate, const uint8_t extruder)
{
  ...
  //luojin	apply_rotation_xyz(Setting.plan_bed_level_matrix, &x, &y, &z);
  }
  #endif
#endif
  block->point_x = x;
  ...
  if(system_infor.Auto_Levele_Flag==1)
  {
	...
	if((system_infor.sd_print_status==0x02||system_infor.serial_printf_flag == 1)&&system_infor.auto_leveling_calculate_enable==ENABLE)//��ӡʱ�س鳤�ȴ���15mm���������ϵķ���
	{
...
{    	  
    ...
    if((Autohome.count%Autohome.period[axis] == 0)&&(Autohome.flag[axis])) 
    {
     ...
     //  my_printf("steps=%d,s=%d,t=%d,n=%d\r\n",Autohome.step[axis],endstop_status[axis],touch_end_signal[axis],normal_end_signal[axis]);
#ifdef BOARD_A30_MINI_S
       ...
       if(Autohome.step[axis] == 4) 
       {
           ...
           if(endstop_status[axis] != normal_end_signal[axis])
              // if(GPIO_ReadInputDataBit(Endstop,Min_X) != normal_end_signal[axis])
           {
 ...
 void Axis_home_control(void)
 {
  ...
  void X_go_origin(float origin_coordinate)
{
...
void matrix_3x3_transpose1(matrix_3x3 *dst, matrix_3x4 original) {
...
void plan_get_position(vector_3 *position) {
   ...
   matrix_3x3_transpose1(&inverse,  Setting.plan_bed_level_matrix);
