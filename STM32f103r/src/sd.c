...
void SD_DMA_RxConfig(uint32_t *BufferDST, uint32_t BufferSize)
{
  ...
  DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)&SDIO->FIFO;  //外设地址，fifo
  ...
  DMA_InitStructure.DMA_Priority = DMA_Priority_High;	 //通道优先级高DMA_Priority_VeryHigh;//
...
SD_Error SD_Init(void)
{
  ...
  //SD_DeInit();
  SD_LowLevel_Init();//SD卡外设引脚初始化
  SDIO_DeInit();  //SD卡寄存器默认值
...
SD_Error SD_WaitReadOperation(void)
{
  ...
  TransferError = SD_OK;  
  while ((TransferEnd == 0x00) && (TransferError == SD_OK) && (timeout > 0)) 
  {
    timeout--;
    SD_ProcessIRQSrc();
    //printf("SDIO->STA : %d \r\n",SDIO->STA);
...
void SD_ProcessDMAIRQ(void)
{
  ...
  else if(DMA2->ISR & DMA2_FLAG_TE4)
  {
    TransferError = SD_SDIO_DMA_ERROR;
    DMA_ClearFlag(DMA2_FLAG_TC4 | DMA2_FLAG_TE4 | DMA2_FLAG_HT4 | DMA2_FLAG_GL4);
  }
...
static SD_Error FindSCR(uint16_t rca, uint32_t *pscr)
{
  ...
  while (!(SDIO->STA & (SDIO_FLAG_RXOVERR | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DTIMEOUT | SDIO_FLAG_DBCKEND | SDIO_FLAG_STBITERR)) && (timeout > 0))
  {
